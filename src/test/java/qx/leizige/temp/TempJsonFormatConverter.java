 ...
 * json格式+类型转换
 */
public class TempJsonFormatConverter {
	...
	public static JSONObject exchangeJson(JSONObject sourceJsonObj, List<JsonConvertTemplate> templateList) {
		...
		templateList.stream().filter(v -> StringUtils.isEmpty(v.getOldPath())).forEach(template -> {
			if (StringUtils.isEmpty(template.getOldPath()) && StringUtils.isNotEmpty(template.getNewPath())) {
				...
				Object newValue = convertTo(template.getTargetType(), template.getValueExpression(), defaultValue);
				appendJsonPath(newJson, template.getNewPath(), newValue);
			}
		});
		...
		for (Map.Entry<String, Object> pathEntry : paths.entrySet()) {
			templateList.stream().filter(v -> StringUtils.isNotEmpty(v.getOldPath())).forEach(template -> {
				if (StringUtils.isNotEmpty(template.getOldPath()) && pathEntry.getKey()
	...
	private final static String EMPTY = "";        //路径中的.替换成/
	...
	private static void convertPathToRegexPath(List<JsonConvertTemplate> templateList) {
		templateList.forEach(template -> {
			if (StringUtils.isNotBlank(template.getOldPath())) {
				...
				String convertOldPath = "/" + template.getOldPath().replaceAll(OLD_PATH_REGEX, OLD_PATH_REPLACEMENT)
						.replace(DOT, OLD_PATH_REPLACE);
				template.setOldPath(convertOldPath);
			}
			//针对目标路径进行转换    /order_list[1].sku_name
			String convertNewPath = "/" + template.getNewPath().replaceAll(NEW_PATH_REGEX, NEW_PATH_REPLACEMENT)
					.replace(DOT, SLASH);
			template.setNewPath(convertNewPath);
//			if (StringUtils.isBlank(template.getOldPath())) {
//				//针对目标路径进行转换    /order_list[1].sku_name
//				String convertNewPath = "/" + template.getNewPath().replaceAll("\\[", SLASH)
//						.replaceAll("\\]", EMPTY)
//						.replace(DOT, SLASH);
//				template.setNewPath(convertNewPath);
//			}
	...
	private static Object convertTo(String targetTypeName, String valueExpression, Object val) {
		...
		case "LocalDateTime":
			...
			break;
		case "jsonObject":
			newVal = JSON.parseObject(val.toString().trim());
			break;
		case "jsonArray":
			newVal = JSON.parseArray(val.toString().trim());
			break;
		default:
			newVal = val;
			break;
